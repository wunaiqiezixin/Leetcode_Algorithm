# 106. 从中序与后序遍历序列构造二叉树
[力扣题目链接](https://leetcode.cn/problems/construct-binary-tree-from-inorder-and-postorder-traversal/description/)

---
## 🧠思路：

完整代码：
### ***C:***
```C
/**
 * Definition for a binary tree node.
 * struct TreeNode {
 *     int val;
 *     struct TreeNode *left;
 *     struct TreeNode *right;
 * };
 */
struct TreeNode* buildTree(int* inorder, int inorderSize, int* postorder, int postorderSize) {
    //第一步：判断数组是否为空
    if (!inorderSize) return NULL;
    //第二步：后序数组的最后一个元素为当前根节点
    struct TreeNode* curRoot = (struct TreeNode*)malloc(sizeof(struct TreeNode));
    int values = postorder[postorderSize - 1];
    curRoot->val = values;
    //第三步：在中序数组中找分割点
    int index = 0;
    while (inorder[index] != values)
        index++;
    
    //第四步：切割中序数组
    //左中序数组起点地址为inorder, 长度为 index
    //右中序数组起点地址为inorder+index+1, 长度为 inorderSize - index - 1

    //第五步：切割后序数组--->两个区间的长度与中序的两个区间同
    //左后序数组的起点地址为postorder, 长度为 index
    //右后序数组的起点地址为postorder + index , 长度为 inorderSize - index - 1

    //第六步：递归，左孩子对左区间，右孩子对右区间
    curRoot->left = buildTree(inorder, index, postorder, index);
    curRoot->right = buildTree(inorder + index + 1,inorderSize - index - 1 , postorder + index, inorderSize - index - 1);
    
    return curRoot;
}
```
### ACM模式
[P0640:根据二叉树中后序序列建树](http://cxsjsxmooc.openjudge.cn/dspythonbookqh2/P0640/)

完整代码：
### ***C:***
```C
#define _CRT_SECURE_NO_WARNINGS
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
typedef struct TreeNode {
	char val;
	struct TreeNode* left;
	struct TreeNode* right;
}TreeNode;
//前序遍历
void preOrder(TreeNode* root, char* ret, int* size)
{
	if (root == NULL)
		return;
	ret[(*size)++] = root->val;
	preOrder(root->left, ret, size);
	preOrder(root->right, ret, size);
	return;
}

//根据中后序列建树
TreeNode* buildTree(char* in, int inSize, char* po, int poSize)
{
	if (inSize == 0)
		return NULL;
	TreeNode* root = (TreeNode*)malloc(sizeof(TreeNode));
	root->val = po[poSize - 1];
	int index = 0;
	while (in[index] != po[poSize - 1])
		index++;
	root->left = buildTree(in, index, po, index);
	root->right = buildTree(in + index + 1, inSize - index - 1,
		                    po + index, inSize - index - 1);
	return root;
}

int main(void)
{
	char in[30], po[30];
	scanf("%s %s", in, po);
	int inSize = strlen(in), poSize = strlen(po);
	TreeNode* root = buildTree(in, inSize, po, poSize);
	char pre[30];
	int size = 0;
	preOrder(root, pre, &size);
	pre[size] = '\0';
	printf("%s", pre);
}
```
